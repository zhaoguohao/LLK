...
public class Packager {
    ...
    static void HandleExampleBundle(BuildTarget target) {
        ...
        BuildPipeline.PopAssetDependencies();
        /// --生成LLKPanel素材绑定---
        //一个llk.unity就包括了llkPanel, 
        BuildPipeline.PushAssetDependencies();
        mainAsset = AssetDatabase.LoadMainAssetAtPath("Assets/LuaFramework/Examples/Builds/LLKPanel.prefab");
        assetfile = assetPath + "llk" + AppConst.ExtName;
        UnityEngine.Debug.Log("asset file is : " + assetfile); 
        addis = new Object[2];
        addis[0] = LoadAsset("LLKItem.prefab");
        addis[1] = LoadAsset("icon.prefab");
        BuildPipeline.BuildAssetBundle(mainAsset, addis, assetfile, options, target);
        BuildPipeline.PopAssetDependencies();
        ///-------------------------------刷新---------------------------------------      
        BuildPipeline.PopAssetDependencies();//停止对第一个share.prefab的依赖
    }
    [MenuItem("zha/test")]
    public static void zhatest()
    {
        //下次看看如何自动一起打包.
       Object m = AssetDatabase.LoadMainAssetAtPath("Assets/LuaFramework/Examples/Builds/LLKPanel.prefab");
        Object[] arr = {m}; 
         Object[] jj = EditorUtility.CollectDependencies(arr);
         foreach(Object oo in jj)
         {
             UnityEngine.Debug.Log("oo is " + oo.ToString());
         }
    }
    /// <summary>
    /// 处理Lua文件
